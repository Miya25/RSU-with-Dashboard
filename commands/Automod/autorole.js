  ...
  run: async (client, message, args) => {
     if (message.member.hasPermission("MANAGE_SERVER")) {
      if (message.content.includes("@everyone")) {
        return message.reply(`${emoji.sucess} Everyone is already automatically given by discord`);
      }
...
if(!args[0])
{
  ...
  .setDescription(`You Didnt Gave Me Role To Add When A Member Joins The Server`)
...
if(args[0] == "disable" || args[0] == "off")
{
  ...
  return message.reply(`${emoji.sucess} Done i have Disabled auto role in your server enable it by adding any role`);
}
else {
