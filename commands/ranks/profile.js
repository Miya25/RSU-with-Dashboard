...
const emoji = require('../../emoji.json')
  ...
  run: async (client, message, args) => {
    ...
    if (rawLeaderboard.length < 1) return message.channel.send(`${emoji.error} Nobody's in leaderboard yet.`);
	...
	ctx.closePath();
	ctx.clip();
    const attachment = new Discord.MessageAttachment(canvas.toBuffer(), `hydroxlbrank.jpg`);
