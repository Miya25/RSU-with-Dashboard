    ...
    collector.on('collect', async (msg) => {
        ...
        if (step == 1) {
            ...
            if (parseInt(winners) > 10) {
                message.reply(`${emoji.error} You cannot have more than 10 winners!`);
        ...
        if (reason == 'done' && giveaway.requirements.enabled) {
            ...
            rcollector.on('end', async (msg, r) => {
                ...
                if (r == 'done') {
                    ...
                    await message.channel.send(`${emoji.sucess} Created a giveaway!!`).then(m => setTimeout(() => m.delete(), 2000));
                }
            });
        }
        else {
